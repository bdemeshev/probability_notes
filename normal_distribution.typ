...
то ожидание $EE(Y_1 Y_2 dot dots dot Y_n)$ является суммой всех возможных произведений ожиданий $(m_i)$ и ковариаций $(c_i j)$, в которых каждый индекс от $1$ до $n$ упомянут ровно один раз. Для примера,
$
    EE(Y_1) = m_1, \
    EE(Y_1 Y_2) = m_1 m_2 + c_(12), \
    EE(Y_1 Y_2 Y_3) = m_1 с_(23) + m_2 с_(13) + m_3 с_(12) +  m_1 m_2 m_3, \
...
Переход к центрированным величинам, $Y_i = m_i + X_i$, и раскрытие скобок показывает, что функция $h()$ является многочленом относительно переменных $m_1$, $m_2$, dots, $m_n$.
...
$
  ...
  + m_1 m_2 m_3 + EE(X_1 X_2 X_3).
$
Докажем теперь, что от дисперсий $c_(i i)$ ожидание $h(m, C)$ не зависит. Для этого прибавим к первой величине $Y_1$ помеху $t W$, где $W$ — стандартная нормальная величина, $W disteq Norm(0, 1)$, независимую ни от одной из величин $Y_1$, $Y_2$, dots $Y_n$.
$
