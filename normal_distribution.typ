...
// заголовок нужен потому, что это будет название главы в книжке!


== От аксиом Гершеля-Максвелла к дифференциальному уравнению

Стандартный учебник по теории вероятностей, начинает главу про нормальное распределение с определения,

#definition[Классическое определение нормального распределения][Случайная величина имеет нормальное распределение с ожиданием $mu$ и дисперсией $sigma^2$, если её функция плотности имеет вид
$
f(x) = frac(1, sqrt(2 pi)) exp(- (x-mu)^2/(2 sigma^2)).  
$ 
]
У любого небезразличного читателя сразу возникнет вопрос: шо? откуда тут взялись $pi$, экспонента и корень? Поэтому давайте сейчас забудем эту формулу и попробуем переоткрыть её. 
...
Говорят, что компоненты вектора $(X, Y)$ имеют  нормальное распределение, симметричное относительно нуля, если вектор удовлетворяет двум аксиомам: 

1. Закон распределения вектора $(X, Y)$ инвариантен к повороту на любой фиксированный угол относительно точки $(0, 0)$. 
...
#definition[Нормальное центрированное распределение][Случайная величина $X$, чья функция плотности $f(x)$ удовлетворяет дифференциальному уравнению
$
  frac(x f(x), f'(x)) = -sigma^2,
$
...
Чтобы быть в числе благородных донов остаётся доказать, что из условия единичной площади под функцией плотности следует, что нормировочная константа равна именно $frac(1, sqrt(2 pi sigma^2), style: "horizontal")$.
...
Связь экспоненциальным распределением. 
...
todo: картинка хитрой лисы?

Рассмотрим многомерный нормальный вектор $Y disteq Norm(m, C)$.

Давайте научимся находить математические ожидания от произвольных одночленов.
Например, чему равно ожидание $EE(Y_1 Y_2 Y_3 Y_4)$?

Для удобства явно выпишем вектор ожиданий $m$ и ковариационную матрицу $C$:

$
  Y disteq Norm(vec(m_1, m_2, m_3, m_4), 
  mat(c_(11), c_(12), c_(13), c_(14);
  c_(21), c_(22), c_(23), c_(24);
  c_(31), c_(32), c_(33), c_(34);
  c_(41), c_(42), c_(43), c_(44))).
$

Начнём с самых простых одночленов! Чему равно ожидание $EE(Y_1)$?

Здесь ответ простой, и нормальность распределения вовсе не нужна, мы сами ввели обозначение 
$
EE(Y_1) = m_1.
$

Второй шаг. Чему равно ожидание $EE(Y_1 Y_2)$?

В силу определения ковариации, $Cov(Y_1, Y_2) = EE(Y_1 Y_2) - EE(Y_1) EE(Y_2)$, получаем формулу
$
  EE(Y_1 Y_2) = m_1 m_2 + c_(12).
$
Нормальность пока, по-прежнему, не использовали. Эта формула верна для произвольно распределенных величин с $EE(Y_i) = m_i$ и $Cov(Y_i, Y_j) = c_(i j)$.

Третий шаг. Чему равно ожидание $EE(Y_1 Y_2 Y_3)$?

Для удобства центрируем величины, $Y_i = m_i + X_i$, у вектора $X = (X_1, X_2, X_3, X_4)$ будут нулевые математические ожидания и исходная ковариационная матрица,
$
    X disteq Norm(vec(0, 0, 0, 0), 
  mat(c_(11), c_(12), c_(13), c_(14);
  c_(21), c_(22), c_(23), c_(24);
  c_(31), c_(32), c_(33), c_(34);
  c_(41), c_(42), c_(43), c_(44))).
$
Используем центрирование и раскрываем скобки!
$
  EE(Y_1 Y_2 Y_3) = EE((m_1 + X_1)(m_2 + X_2)(m_3 + X_3)) =  \
  = m_1 EE(X_2 X_3) + m_2 EE(X_1 X_3) + m_3 EE(X_1 X_2) + m_1 m_2 EE(X_3) + m_1 EE(X_2) m_3 + EE(X_1) m_2 m_3 + \
  + m_1 m_2 m_3 + EE(X_1 X_2 X_3).
$
В силу центрированности величин $X_i$ с ними проще: $EE(X_i) = 0$ и $EE(X_i X_j) = c_(i j)$.
Упрощаем наше выражение:
$
  EE(Y_1 Y_2 Y_3) = m_1 с_(23) + m_2 с_(13) + m_3 с_(12) +  m_1 m_2 m_3 + EE(X_1 X_2 X_3).
$
Теперь воспользуемся симметрией! todo: ходулочник или цветок в воде с отражением
Повернём вектор $X$ на $180°$, получим вектор $W = (-X_1, -X_2, -X_3)$. 
У вектора $W$ снова будут нулевые ожидания и та же исходная ковариационная матрица в силу того, что $Cov(-X_i, -X_j) = Cov(X_i, X_j)$,
$
    W disteq Norm(vec(0, 0, 0, 0), 
  mat(c_(11), c_(12), c_(13), c_(14);
  c_(21), c_(22), c_(23), c_(24);
  c_(31), c_(32), c_(33), c_(34);
  c_(41), c_(42), c_(43), c_(44))).
$
С одной стороны, мы видим, что векторы $X$ и $W$ распределены одинаково, $W disteq X$, поэтому 
$
EE(W_1 W_2 W_3) = EE(X_1 X_2 X_3).
$
С другой стороны, 
$
EE(W_1 W_2 W_3) = EE((-X_1)(-X_2)(-X_3)) = EE(- X_1 X_2 X_3) = - EE(X_1 X_2 X_3).
$
Одновременно эти два равенства возможны, только если $EE(X_1 X_2 X_3) = 0$.

Ура, мы получили ожидаемое значение для произведения трёх игреков:
$
  EE(Y_1 Y_2 Y_3) = m_1 с_(23) + m_2 с_(13) + m_3 с_(12) +  m_1 m_2 m_3.
$
Здесь мы уже пользовались симметричностью нормального распределения. 

Упражнение. Придумайте вектор с ожиданиями $EE(Y_i) = m_i$ и ковариацией $Cov(Y_i, Y_j) = c_(i j)$, для которого формула $EE(Y_1 Y_2 Y_3) = m_1 с_(23) + m_2 с_(13) + m_3 с_(12) +  m_1 m_2 m_3$ не верна. 


Попытаемся заметить закономерности и построить догадку для $EE(Y_1 Y_2 Y_3 Y_4)$.
...
$
    ...
    EE(Y_1 Y_2 Y_3) = m_1 с_(23) + m_2 с_(13) + m_3 с_(12) +  m_1 m_2 m_3.
$
Что мы видим общего у полученных трёх формул?

Во-первых, везде в правой части только многочлены от $m_i$ и $c_(i j)$. Не возникают ни дроби, ни корни, ни арксинусы, ни тангенсы, ни прочие ужасные функции. 
Это немножко ожидаемо, если вспомнить про размерности величин. 
Скажем, если $Y_1$ измеряется в килограммах, $Y_2$ — в сантиметрах, а $Y_3$ — в литрах, то величина $EE(Y_1 Y_2 Y_3)$ обязана измерятся в $"кг" dot "см" dot "л"$. 
И слагаемое вида $m_1^3 / c_(23)$ невозможно в силу того, что измеряется в $"кг"^3 / ("см" dot "л")$.
Напомним, что $c_(23)$ измеряется в $"кг" dot "л"$.
Впрочем, соображения размерностей не исключают слагаемых вида $m_2 c_(12) c_(23) / c_(22)$.

Во-вторых, нигде не участвуют дисперсии. Я не знаю интуитивного аргумента, почему бы дисперсиям там не оказаться. 

В-третьих, все коэффициенты многочлена в правой части формулы равны единице. Соображения симметрии, конечно, гарантируют нам, что должны быть равны коэффициенты при слагаемых $m_1 c_(23)$ и $m_2 c_(13)$. 
Но здесь равны все коэффициенты и более того, равны единице. 

В-четвертых, в правой части можно найти все возможные одночлены, где упомянуты ровно по одному разу все индексы левой части. Например, для $EE(Y_1 Y_2 Y_3)$ правая часть содержит все возможные слагаемые с одной единицей, одной двойкой и одной тройкой в индексе. 

Если предположить, что все эти подмеченные особенности сохраняются дальше, то искомое ожидание $EE(Y_1 Y_2 Y_3 Y_4)$ обязано равняться,
$
 EE(Y_1 Y_2 Y_3 Y_4) = m_1 m_2 m_3 m_4 + \
  + m_1 m_2 c_(34) + m_1 m_3 c_(14) + m_1 m_4 c_(23) + m_2 m_3 c_(14) + m_2 m_4 c_(13) + m_3 m_4 c_(12) + \
  + c_(12) c_(34) + c_(13) c_(24) + c_(14) c_(23).
$
И это оказывается верно! Давайте сформулируем теорему Иссерлиса. 
По моему мнению, проще сформулировать её текстом, чем формулой. 

#theorem["Теорема Иссерлиса"][Если вектор $Y$ имеет многомерное нормальное распределение $Norm(m, C)$, 
то ожидание $EE(Y_1 Y_2 dot dots dot Y_n)$ является суммой всех возможных произведений ожиданий $(m_i)$ и ковариаций $(c_(i j))$, в которых каждый индекс от $1$ до $n$ упомянут ровно один раз. Для примера,
$
    EE(Y_1) = m_1, \
    EE(Y_1 Y_2) = m_1 m_2 + c_(12), \
    EE(Y_1 Y_2 Y_3) = m_1 с_(23) + m_2 с_(13) + m_3 с_(12) +  m_1 m_2 m_3, \
...
#proof[Доказательство через добавление помех][Многомерное нормальное распределение полностью определяется вектором ожиданий $m$ и ковариационной матрицей $C$, поэтому $EE(Y_1 Y_2 dot dots dot Y_n)$ должно быть функцией от $m$ и $C$, 
$
  EE(Y_1 Y_2 dot dots dot Y_n) = h(m, C).
$

Переход к центрированным величинам, $Y_i = m_i + X_i$, и раскрытие скобок показывает, что функция $h()$ является многочленом относительно переменных $m_1$, $m_2$, ..., $m_n$.
Например, для трёх переменных,
$
  EE(Y_1 Y_2 Y_3) = EE((m_1 + X_1)(m_2 + X_2)(m_3 + X_3)) =  \
  = m_1 EE(X_2 X_3) + m_2 EE(X_1 X_3) + m_3 EE(X_1 X_2) + m_1 m_2 EE(X_3) + m_1 EE(X_2) m_3 + EE(X_1) m_2 m_3 + \
  + m_1 m_2 m_3 + EE(X_1 X_2 X_3).
$
Докажем теперь, что от дисперсий $c_(i i)$ ожидание $h(m, C)$ не зависит. Для этого прибавим к первой величине $Y_1$ помеху $t W$, где $W$ — стандартная нормальная величина, $W disteq Norm(0, 1)$, независимую ни от одной из величин $Y_1$, $Y_2$, ..., $Y_n$.
$
  EE((Y_1 + t W)Y_2 dot dots dot Y_n) = EE(Y_1 Y_2 dot dots dot Y_n) + t EE(W dot Y_2 dot dots dot Y_n) = \
  = EE(Y_1 Y_2 dot dots dot Y_n) + t EE(W) EE(Y_2 dot dots dot Y_n).
$
Ожидание $EE(W)$ равно нулю, поэтому выходит, что прибавление помехи не повлияло на значение искомого ожидания. 
Однако прибавление помехи $t W$ к величине $Y_1$ увеличивает дисперсию первого элемента вектора, не трогая ожидания $(m_i)$ и остальные элементы ковариационной матрицы $C$. Следовательно, изменение дисперсии никак не влияет на итоговое значение ожидания $h(m, C)$.

Теперь докажем, что $h(m, C)$ является многочленом от каждой ковариации $(c_(i j))$. 
Для этого прибавим помеху $t W$ к *двум* первым величинам,
$
EE((Y_1 + t W)(Y_2 + t W) Y_3 dot dots dot Y_n) = EE(Y_1 Y_2 Y_3 dot dots dot Y_n) + t^2 EE(W^2) EE(Y_3 Y_4 dot dots dot Y_n) + \
+ t EE(W) EE(Y_1 Y_3 dot dots dot Y_n) + t EE(W) EE(Y_2 Y_3 dot dots dot Y_n).
$
В силу того, что $W disteq Norm(0, 1)$ можем подставить ожидание $EE(W) = 0$ и дисперсию $Var(W) = EE(W^2) = 1$,
$
  EE((Y_1 + t W)(Y_2 + t W) Y_3 dot dots dot Y_n) = EE(Y_1 Y_2 Y_3 dot dots dot Y_n) + t^2 EE(Y_3 Y_4 dot dots dot Y_n).
$
Внесение помехи не влияет на вектор ожиданий $m$ и добавляет ровно величину $t^2$ в ковариацию $c_(12)$,
$
  Cov(Y_1 + t W, Y_2 + t W) = Cov(Y_1, Y_2) + t dot 0 + t dot 0 + t^2 dot 1. 
$
Следовательно, зависимость функции $h(m, C)$ от ковариации $c_(12)$ между первыми двумя величинами $Y_1$ и $Y_2$ линейная. Остальные ковариации ничем не лучше и не хуже :)

К этому моменту мы доказали, что искомая функция $h(m, C)$ является многочленом от ожиданий $(m_i)$ и ковариаций $(c_(i j))$. Остаётся доказать, что все нужные одночлены, где каждый нижний индекс упомянут ровно один раз, присутствуют с коэффициентом $1$, а все лишние одночлены — полностью отсутствуют. 

Покажем охоту за коэффициентами многочлена на примере :)

Как найти коэффициент при одночлене $m_1 c_(23) c_(45)$ в многочлене $EE(Y_1 Y_2 Y_3 Y_4 Y_5)$?

Мы просто занулим все остальные одночлены! Для этого возьмём $Y_1$ тождественно равное константе $m_1$, пару стандартных нормальных величин $Y_2$, $Y_3$, тождественно равных друг другу, $Y_2 = Y_3$, и не зависящих ни от чего другого, и пару стандартных нормальных величин $Y_4$, $Y_5$, тождественно равных друг другу, $Y_4 = Y_5$, и не зависисящих ни от чего другого. Мы специально подстроили, что все ковариации $c_(i j)$ равны $0$ кроме $c_(23) = 1$ и $c_(45) = 1$. И специально подстроили, что все ожидания $m_i$  равны $0$ кроме $m_1$. Тем самым мы оставили в многочлене только нужный одночлен с неизвестным коэффициентом $k$ спереди!
$
  EE(Y_1 Y_2 Y_3 Y_4 Y_5) = k m_1 c_(23) c_(45). 
$
В силу того, что $m_1 = 1$, $c_(23) = 1$, $c_(45) = 1$, правая часть равна $k$.
Левая часть равна 
$
  EE(Y_1 Y_2 Y_3 Y_4 Y_5) = EE(Y_1 Y_2^2 Y_4^2) = EE(Y_1) EE(Y_2^2) EE(Y_4^2) = 1 dot 1 dot 1. 
$
Отсюда искомый коэффициент $k$ перед одночленом равен $k=1$.

Дотошный читатель может возразить, что полно других многочленов, дающих нужное единичное ожидание $EE(Y_1 Y_2 Y_3 Y_4 Y_5)$, например, 
$
  EE(Y_1 Y_2 Y_3 Y_4 Y_5) = 0.5 m_1^3 c_(23) c_(45)^7 + 0.5 m_1 c_(23)^2 c_(45)^3.
$
От таких многочленов нас спасают соображения размерности. 
В предложенном контр-примере размерность левой и правой части не совпадает. 
Скажем, если все $Y_i$ измеряются в килограммах, то левая часть измеряется в $("кг")^5$, а в правой части даже не получится сложить два слагаемых $0.5 ("кг")^(11) + 0.5 ("кг")^6$. 
Также от гипотетической возможности многочленов с повтором какого-то нижнего индекса $i$ можно избавиться, мысленно увеличив соответствующую величину $Y_i$ в $t$ раз. При таком изменении искомое ожидание  $h(m, C)$ должно увеличиться в $t$ раз, значит каждый индекс должен быть упомянут в каждом одночлене ровно один раз. 
]
